...
/// Drop-in alternative to [GeminiService] for text generation, using the
/// DeepSeek API (OpenAI-compatible `chat/completions`) instead of Gemini.
/// Same public method signatures/prompts as GeminiService so it can be
...
class DeepSeekService {
  ...
  Future<Map<String, dynamic>> generateLesson(String topic, String level) async {
    if (!_isInitialized) throw Exception('DeepSeekService not initialized');
    ...
    **Logique de Traitement de la Demande Utilisateur :**
    ...
    L'API génère le contenu sous forme de JSON strict.
    ...
    **Structure JSON attendue :**
    ...
    try {
      final responseText = await _complete(prompt);
